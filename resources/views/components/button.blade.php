...
@php
    ...
    $appliedClasses = [$baseClasses, $variants[$variant] ?? $variants['default'], $sizes[$size] ?? $sizes['default']];
@endphp

@if($attributes->has('href'))
    <a {{ $attributes->merge(['data-slot' => 'button'])->class($appliedClasses) }}>
        {{ $slot }}
    </a>
@else
    <button {{ $attributes->merge(['data-slot' => 'button'])->class($appliedClasses) }}>
        {{ $slot }}
    </button>
@endif
